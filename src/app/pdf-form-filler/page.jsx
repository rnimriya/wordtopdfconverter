...
function PDFFormFiller() {
  ...
  const handleExecute = async () => {
    ...
    try {
      setProgress(50);
      const blob = await editPdfLocal(file, { task: 'form', text });
      setProgress(90);
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      const downloadName = file.name.replace(/\.[^/.]+$/, '') + '-form.pdf';
  ...
  const controls = (
    <div className="space-y-4">
      ...
      <textarea value={text} onChange={(e) => setText(e.target.value)} placeholder="Form field value..." className="w-full bg-slate-900 border border-slate-700 text-slate-300 rounded-lg p-3 text-sm focus:ring-primary-500 focus:border-primary-500 outline-none transition-all resize-none h-20" />
      <button onClick={handleExecute} className="w-full glass-button-primary text-xs flex items-center justify-center space-x-2" disabled={!file || isExecuting}>
        {isExecuting ? (
          ...
          <><Type className="h-4 w-4" /><span>PDF Form Filler</span></>
        )}
      </button>
    </div>
  );
    ...
    "url": "https://wordtopdfconverter.online/pdf-form-filler",
  ...
  const seoContent = (
    <div className="prose prose-invert max-w-none space-y-8">
      <div className="space-y-4">
        <h1 className="text-3xl font-bold font-display text-white">PDF Form Filler Quickly and Securely</h1>
      ...
      <div className="space-y-4">
        <h2 className="text-2xl font-bold text-white border-b border-slate-800 pb-2">How to PDF Form Filler Online</h2>
