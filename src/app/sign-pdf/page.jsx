...
function SignPDF() {
  ...
  const controls = (
    <div className="space-y-4">
      ...
      <textarea value={text} onChange={(e) => setText(e.target.value)} placeholder="Type your full name to sign..." className="w-full bg-slate-900 border border-slate-700 text-slate-300 rounded-lg p-3 text-sm focus:ring-primary-500 focus:border-primary-500 outline-none transition-all resize-none h-20" />
      <button onClick={handleExecute} className="w-full glass-button-primary text-xs flex items-center justify-center space-x-2" disabled={!file || isExecuting}>
        {isExecuting ? (
          ...
          <><PenTool className="h-4 w-4" /><span>Sign PDF</span></>
        )}
      </button>
    </div>
  );
    ...
    "@type": "WebApplication",
    "name": "Sign PDF",
    "url": "https://wordtopdfconverter.online/sign-pdf",
    "description": "A secure, fast, and free online tool to sign your PDF files with 100% precision.",
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">You sure can! Our website is fully optimized for mobile devices, so you can easily sign your online using your iPhone or Android browser—no extra apps required.</p>
          </div>
          <div>
